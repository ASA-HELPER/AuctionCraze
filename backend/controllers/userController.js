...
export const register = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!allowedImageFormats.includes(profileImage.mimetype)) {
  ...
  if (!userName || !email || !phone || !password || !address || !role) {
    return next(new ErrorHandler("Please fill full all the fields.", 400));
  }
  if (role === "Auctioneer") {
  ...
  const isRegistered = await User.findOne({ email: email.toLowerCase() });
  if (isRegistered) {
